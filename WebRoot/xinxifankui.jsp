...
if (a.equals("insert")) {
    // 执行插入信息反馈模块,表名：xinxifankui
    HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据

    String charuid = new CommDAO().insert(request, "xinxifankui", ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    // 插入数据完成，弹出页面并跳转到指定页面
    ...
    // 执行更新信息反馈模块,表名：xinxifankui
    HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据

    String charuid = new CommDAO().update(request, "xinxifankui", ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法

    new CommDAO().commOper("update xinxifankui set fankuizhuangtai='已回复' where id='" + charuid + "'");
    ...
    // 获取一行数据
    Map map = Query.make("xinxifankui").find(Request.get("id"));

    new CommDAO().delete(request, "xinxifankui"); // 执行删除命令
